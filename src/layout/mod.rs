...
impl<W: LayoutElement> Layout<W> {
        ...
        self.monitor_set = match mem::take(&mut self.monitor_set) {
            MonitorSet::Normal {
                ...
                for source_idx in 0..monitors.len() {
                    ...
                    //
                    // The second case can also be hit with a workspace switch still active
                    // (nothing was moved to the new output, but e.g. an overview gesture is in
                    // progress) — there, len == 2 may include a hidden workspace rather than two
                    // empty ones, and clean_up_workspaces asserts no switch is active. Defer to
                    // the cleanup that runs when the switch finishes.
                    if stopped_ws_switch
                        || (source.workspace_switch.is_none()
                            && source.options.layout.empty_workspace_above_first
                    ...
                    for (idx, ws) in mon.workspaces.iter_mut().enumerate() {
                        if ws.has_window(window) {
                                ...
                                && mon.workspace_switch.is_none()
                            {
                                ...
                                mon.shift_hidden_original_indices_for_removal(idx);
    ...
    pub fn set_workspace_name(&mut self, name: String, reference: Option<WorkspaceReference>) {
        ...
        if let MonitorSet::Normal { monitors, .. } = &mut self.monitor_set {
            ...
            // empty, doubling as the hidden-block guard) — restore an empty one
            // after it.
                ...
                for mon in monitors {
                    if mon.sticky.has_window(window) {
                        mon.sticky.store_unmap_snapshot_if_empty(
                            ...
                            xray,
                            xray_has_blocked_out_layers,
                            XrayPos::new(Point::from((0., 0.)), zoom),
                            window,
                        );
                        return;
                    }
